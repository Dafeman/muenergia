    ...
    const uint32_t& boosterPack, const bool& threadsActivated)
{
  ...
  /**If good */
#if defined(EMBEDDED_MODE)
  //tivaWare.LED.colorSetRGB(0, 0xFFFF, 0);
  //tivaWare.LED.blinkRateSetRGB(1.0f);
#endif
...
#if defined(EMBEDDED_MODE)
    TivaWareController::getInstance().UART.write(strand(node->getName()));
    TivaWareController::getInstance().UART.printf("\n");
#endif
  }
}
